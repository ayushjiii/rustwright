...
def default_output_path(args: argparse.Namespace) -> Path:
    ...
    return ROOT / ".benchmark-data" / "results" / (
        f"webvoyager-{percent}pct-seed{args.seed}-{args.repetitions}x{args.iterations}.json"
    )
...
def run_impl(args: argparse.Namespace, implementation: str) -> dict[str, Any]:
    env = {**os.environ, "WEBVOYAGER_ITERATIONS": str(args.iterations)}
    command = [
        ...
        "--network-warmup-timeout",
        str(args.network_warmup_timeout),
        "--browser-lifecycle",
    ...
    output = extract_json(combined)
    result = (output.get("results") or [{}])[0]
...
def matrix_metadata(args: argparse.Namespace, implementations: list[str], docker_preflight: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "network_warmup_url": args.network_warmup_url,
        "network_warmup_timeout": args.network_warmup_timeout,
        "browser_lifecycle": args.browser_lifecycle,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run WebVoyager reliability benchmark implementations in separate Docker containers.")
    parser.add_argument("--manifest", default=str(ROOT / ".benchmark-data" / "manifests" / "webvoyager_tasks.json"))
    parser.add_argument("--percentage", type=float, default=30.0)
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--max-tasks", type=int, default=None)
    parser.add_argument("--iterations", type=int, default=int(os.environ.get("WEBVOYAGER_FULL_ITERATIONS", "1")))
    parser.add_argument("--repetitions", type=int, default=1)
    parser.add_argument("--impl", action="append", help="Implementation to run. Defaults to Rustwright and Python Playwright.")
    parser.add_argument("--navigation-timeout", type=float, default=float(os.environ.get("WEBVOYAGER_NAVIGATION_TIMEOUT", "15000")))
    parser.add_argument("--network-warmup-url", default=os.environ.get("WEBVOYAGER_NETWORK_WARMUP_URL", "https://example.com/"))
    ...
    parser.add_argument("--timeout", type=int, default=int(os.environ.get("WEBVOYAGER_CONTAINER_TIMEOUT", "1800")))
    ...
    implementations = args.impl or list(DEFAULT_IMPLS)
